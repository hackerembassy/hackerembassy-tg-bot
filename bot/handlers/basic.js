...
#\`/issue Неплохо было бы иметь карту внутренней сети на вики#\``
    ...
    let report = `📩 Кто-то считает, что нас в спейсе есть проблема, вот что он написал:
"${issueText}"`;
    await this.bot.sendMessage(msg.chat.id, issueText !== undefined ? message : helpMessage);
